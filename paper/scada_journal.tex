...
study, Table~\ref{tab:ongc_paired}); on XJTU-SY ($n{=}10$,
...
\emph{non-destruction}. Across all ten detectors (every one runs on the full
$n{=}6$), $191/600$
...
lead time, collapsing within-bearing sampling factors. All ten detectors run on
the full $n{=}6$ (bearings are long enough to form deep-model sequences). No
...
Table~\ref{tab:ims_paired} gives the run-level test over all ten detectors.
...
therefore not compute-bound for any of the ten detectors, deep models included;
