...
> 3. **The claim that "decimation is never better" has been removed** throughout, as it was contradicted by the paper's own Table 8; the exception is now stated explicitly wherever the result is reported (Reviewer D).
...
**Response.** The reviewer is right and we thank them for catching it; the introduction contradicted §6.4 of the same manuscript. The universal claim has been removed from all six places it appeared (abstract, two locations in §1, §7.2, §7.4 — now deleted — and §9). Wherever the cross-dataset result is now stated, the Isolation Forest result on FEMTO is named as the exception, with its median (−0.14 h), its sign consistency (6 of 6 runs), its raw p (0.031), and the fact that it does not survive Holm correction.

We would add that the revised mechanism analysis (Reviewer G, point 2) now *predicts* this exception rather than merely reporting it: the sufficient-statistic argument applies to the amplitude family, so a spike-robust detector such as Isolation Forest is expected to be the weakest carrier of the aggregation advantage and is the one most likely to reverse.
**Change.** Abstract; §1 ⟦…⟧; §7.2; §9.
